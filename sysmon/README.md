...
- I then copied all of the content of `sysmonconfig.xml` on the Github (once you click the link), unzipped the `Sysmon.zip`, took the `Sysmon64.exe` (this `.exe` was for a Intel/AMD x64 operating system), put that into one folder named `sysmon` and converted it to a `.iso` file. I did this one macOS so the process may be a little different for another OS. This guide was pretty useful: [link](https://www.petenetlive.com/KB/Article/0001554). Lastly, I uploaded it into Proxmox
